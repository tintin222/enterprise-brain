...
import { ChevronDown, ChevronRight, Download, File, Folder, Network, Package, Paperclip as PaperclipIcon, Send, Server, TriangleAlert, Users } from "lucide-react";
...
function TreeView({ node, depth, selected, onSelect }: { node: TreeNode; depth: number; selected: string | null; onSelect: (path: string) => void }) {
  ...
  if (node.file) {
    return (
      <button
        ...
        className={clsx("flex w-full items-center gap-1.5 truncate rounded-md py-1 pr-2 text-left text-[13px]", selected === node.path ? "bg-brand-50 text-brand-700 dark:bg-brand-400/15 dark:text-brand-200" : "text-fg hover:bg-subtle")}
  ...
  return (
    <div>
      {node.name && (
        <button type="button" onClick={() => setOpen((o) => !o)} className="flex w-full items-center gap-1 rounded-md py-1 text-left text-[13px] font-medium text-fg hover:bg-subtle" style={{ paddingLeft: depth * 12 }}>
...
export default function Paperclip() {
  ...
  const departments = scope === "installed" ? (installed.data ?? []).map((d) => ({ id: d.key, name: d.name })) : (catalog.data?.departments ?? []).map((d) => ({ id: d.id, name: d.name }));
  ...
  const [push, setPush] = useState({ paperclipUrl: info.paperclip.url ?? "", paperclipApiKey: "", target: "new_company" as "new_company" | "existing_company", paperclipCompanyId: "" });
  const pushMutation = useMutation({
    ...
    onSuccess: (res) => toast.success("Pushed to Paperclip", { description: `${res.summary.agents} agents, ${res.summary.departments} departments, ${res.summary.routines} routines.` }),
  ...
  return (
    <Page>
      <PageHeader
        ...
        actions={info.paperclip.configured ? <Badge tone="green">Connected to {info.paperclip.url}</Badge> : <Badge tone="neutral">PAPERCLIP_URL not set</Badge>}
      />

      <div className="mb-8 grid gap-4 md:grid-cols-3">
        {[
          { icon: Users, title: "Org chart & governance", text: "Paperclip is the control plane: who reports to whom, tasks, heartbeats, budgets and approvals across all your AI employees." },
          { icon: Network, title: "Hired via hermes_gateway", text: "Each Enterprise Brain agent becomes a Paperclip employee whose adapter calls this instance's Hermes gateway to do the work." },
          { icon: Server, title: "Also an MCP server", text: "Any MCP client (Claude, IDEs, other agents) can search knowledge, list and run agents and read connected systems." },
        ].map((c) => (
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_400px]">
        <Card>
          <CardHeader title="Company package" subtitle="A Paperclip company import: CEO, department leads, specialists, projects and routines." icon={Package} />
          <div className="space-y-5 p-5">
            ...
            <div>
              ...
              {departments.length === 0 ? (
                <p className="text-sm text-muted">{scope === "installed" ? "No departments installed yet — switch to the whole catalog or install one first." : "No departments in the catalog."}</p>
              ) : (
            ...
            {pkg.data && (
              <div className="space-y-4">
                ...
                <div className="grid overflow-hidden rounded-xl border border-line md:grid-cols-[250px_minmax(0,1fr)]">
                  ...
                  <div className="min-w-0">
                    ...
                    <pre className="max-h-[440px] overflow-auto p-3 font-mono text-xs leading-relaxed whitespace-pre-wrap text-fg">{current ? pkg.data.files[current] : "Select a file"}</pre>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader title="Push to Paperclip" subtitle="Imports the package through Paperclip's API and wires each agent's hermes_gateway key." icon={Send} />
            <form onSubmit={submitPush} className="space-y-4 p-5">
              <Field label="Paperclip URL" hint={info.paperclip.configured ? "Defaults to PAPERCLIP_URL." : undefined}>
                {(id) => <input id={id} type="url" className="input" placeholder="http://localhost:3100" value={push.paperclipUrl} onChange={(e) => setPush({ ...push, paperclipUrl: e.target.value })} />}
              </Field>
              <Field label="Paperclip API key" optional>
                {(id) => <input id={id} type="password" autoComplete="off" className="input" value={push.paperclipApiKey} onChange={(e) => setPush({ ...push, paperclipApiKey: e.target.value })} />}
              </Field>
              <div>
              ...
              {push.target === "existing_company" && (
                <Field label="Paperclip company id" required>
                  {(id) => <input id={id} className="input font-mono text-xs" value={push.paperclipCompanyId} onChange={(e) => setPush({ ...push, paperclipCompanyId: e.target.value })} />}
                </Field>
              )}
              <Button type="submit" variant="primary" icon={Send} loading={pushMutation.isPending} disabled={push.target === "existing_company" && !push.paperclipCompanyId}>
                Push
              </Button>
              {pushMutation.data && (
          ...
          <Card>
            ...
            <div className="space-y-5 p-5">
              ...
              <div>
                ...
                <p className="hint">Streamable HTTP. {info.authRequired ? "Send Authorization: Bearer <EB_API_KEY>." : "No key needed in local trusted mode."}</p>
              </div>
            </div>
          </Card>
        </div>
