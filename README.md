...
6. **Settings → Connections** (IT): connect a web service, then *Actions*: import them from its OpenAPI description or a few example calls, name each in plain words, mark which change data, and try them.
7. **Home** (sign in as *Elif Arslan*, accounts payable): give the Invoice Processor work, and follow it as a task.
8. **Approvals by email**: when an AI employee waits for an approval, the people who may decide it get an email with *Approve*, *Reject* and *Correct it first*. Without a mail connection they land in the demo outbox (*Settings → Mailboxes*); their buttons open a page where you confirm. Your menu (top right) → *What reaches me* sets when and where.
9. **Teams and Google Chat** (IT): *Settings → Teams and Chat* walks through connecting your Azure Bot (and downloading the Teams app to upload) or your Google Chat app.
