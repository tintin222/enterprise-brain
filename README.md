...
7. **Approvals by email**: when an AI employee waits for an approval, the people who may decide it get an email with *Approve*, *Reject* and *Correct it first*. Without a mail connection they land in the demo outbox (*Settings → Mailboxes*); their buttons open a page where you confirm. Your menu (top right) → *What reaches me* sets when and where.
8. **Teams and Google Chat** (IT): *Settings → Teams and Chat* walks through connecting your Azure Bot (and downloading the Teams app to upload) or your Google Chat app.
...
Phase 2 is under way: approvals by email, notification preferences and the morning summary, Microsoft Teams and Google Chat, calendars, OAuth 2.0 and client certificates for connections, and MCP servers are built. Coming next: SFTP and shared folders.
