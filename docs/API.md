...
| POST | `/api/companies/:company/tables/:table/intake` | A manager of its department: `{ mailbox, level?: shadow\|supervised\|trusted (default supervised), dryRun? }` → an AI employee that fills the table from email, hired on trial with the viewer as its manager: `{ agent { slug, name, status }, job { duty, picks[], takes[], startsAs[], leaves[], never[], level, levelText } }` (`dryRun`: `{ name, job }`, nobody hired). It reads each email sent to the mailbox, picks out the table's fields, takes the day the email came (in the company's time zone) for a date field about arrival, lets choices with a starting value start there, and adds the record through the Tables connection (a person approves each one while it is Supervised). The table's `can.hire` says whether the viewer may |
