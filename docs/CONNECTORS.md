...
| `tables` | The company's own tables (Apps → a table), kept by Enterprise Brain | tables | find, read, add and change records of each table | stable |

**The company's tables** (a supplier complaints register, a training log: see *Apps*) are a connection too, which Enterprise Brain makes with the first table and keeps in step: each table offers AI employees four actions, `find_<table>` (by words and by field: a choice, a person, a linked record, a date, yes or no), `get_<table>` (by its number), `add_<table>` and `update_<table>` (only the fields given change). Values are checked against the table's fields as they are for people, and an AI employee is told exactly what to correct ("Status: one of Open, In progress, Closed"). Adding and changing are writes, so they follow the AI employee's probation level and are never done in test runs; each change keeps which AI employee made it and in which run. People can't add, change or remove this connection by hand; an AI employee's job names the actions it may use.
